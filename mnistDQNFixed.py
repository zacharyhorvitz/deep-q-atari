...
#TODO: 
#find minibatch size set to 350
#update targed every 2000 stps
#target every 50
...
class DQNSolver:
    ...
    def add_episode(self):
        self.num_episodes+=1
        if self.num_episodes < MEMORY_SIZE:
          self.episode_memory.append([])
        else:
           self.episode_memory[self.num_episodes % MEMORY_SIZE] = []
        

    def episode_remember(self, state, action, reward, next_state, done):
        self.episode_memory[self.num_episodes % MEMORY_SIZE].append((state, action, reward, next_state, done))
